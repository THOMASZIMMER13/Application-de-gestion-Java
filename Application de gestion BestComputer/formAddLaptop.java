...
import java.awt.*;
import javax.swing.*;
public class formAddLaptop extends JFrame implements ActionListener {


    private Connection conn;    //attribut qui memorise la connexion a la base

private JLabel labInventory, labBrand, labModel, labTitle, labDescription, labPrice, labSerialnumber, labReleaseDate, labProcessor, labRam, labStorage, labOs, labScreenSize;

private JTextField jtfInventory, jtfBrand, jtfModel, jtfTitle, jtfPrice, jtfSerialnumber, jtfReleaseDate, jtfProcessor, jtfRam, jtfStorage, jtfOs, jtfScreenSize;
...
setDefaultCloseOperation(DISPOSE_ON_CLOSE);
		...
		this.setTitle("Ajout d'un nouvel ordinateur portable");
...
//		jtfRam=new JTextField();
//		jtfRam.setBounds(250,390,200,30);
//		jtfRam.requestFocusInWindow();
//		pan.add(jtfRam);
...
jtfOs.setToolTipText("Système d'exploitation");
		...
		labScreenSize=new JLabel("Taille du prol' cran (exemple 13.3) :");
		labScreenSize.setBounds(20,510,200,30);
		labScreenSize.setFont(new Font("Arial",Font.BOLD,12));
		labScreenSize.setForeground(Color.white);
		pan.add(labScreenSize);

		jtfScreenSize=new JTextField();
		jtfScreenSize.setBounds(250,510,200,30);
		jtfScreenSize.setToolTipText("Taille de l'écran");
		jtfScreenSize.requestFocusInWindow();
		pan.add(jtfScreenSize);
...
int ram  = Integer.parseInt(value);
        ...
        float screenSize = Float.parseFloat(jtfScreenSize.getText());
        ...
        String sql = "INSERT INTO product(inventory, brand, model, title, description, price, serialnumber, releasedate, processor, ram, storage, os, screensize, producttype) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        stmt.setFloat(13, screenSize);
        stmt.setString(14, productType);

        // Exécution de la requête SQL
        int rowsInserted = stmt.executeUpdate();
        if (rowsInserted > 0) {
          JOptionPane.showMessageDialog(this, "Le laptop a été ajouté à la base de données");
